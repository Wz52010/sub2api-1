...
func RegisterAdminRoutes(
	...
	// 号商模式:管理面只放行六个模块,其余子路径 404(接口层硬关,不只是藏菜单)。
	// 挂在鉴权之后、业务路由之前;主站(默认 false)不挂载,行为完全不变。
	if cfg != nil && cfg.VendorMode.Enabled {
		admin.Use(middleware.VendorModeAdminGuard(v1.BasePath() + "/admin"))
	}
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/:id/diagnose-connection", h.Admin.Account.DiagnoseConnection)
		accounts.GET("/:id/connection-diagnostic", h.Admin.Account.GetConnectionDiagnostic)
		accounts.GET("/:id/transport-health", h.Admin.Account.GetTransportHealth)
